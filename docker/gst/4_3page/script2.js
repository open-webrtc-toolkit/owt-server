...
mix = getParameterByName('mix') === 'false' ? false : false;
...
hasForward = getParameterByName('forward') === 'false' ? false : false;
hasMixed = getParameterByName('mixed') === 'false' ? false : false;
...
var listAnalytics = function (room, ok_cb, err_cb) {
        ...
        if(simulcastAudio === "true"){
            options = {
                ...
                video : [
                    ...
                    {rid: 'q', active: true, scaleResolutionDownBy: 2.0},
...
function removeAll(){
    ...
    var supportedvideocodec=document.getElementById('supportedvideocodec');
...
//create-publish-subscribe-stop

function integrate(createParams, pubParams, subParams) {
    let lStream, rStream, pub, sub;
    _createLocalStream(...createParams)
        .then(localStream => {
            lStream = localStream;
            pubParams.unshift(localStream)
            return _publish(...pubParams)
        })
        .then(([remoteStream, publication]) => {
            pub = publication;
            rStream = remoteStream;
            subParams.unshift(remoteStream)
            return _subscribe(...subParams)
        })
        .then(subscription => {
            sub = subscription;
            let { bitrateMultipliers, frameRates, keyFrameIntervals, resolutions, codecs: videoCodecs } = rStream.capabilities.video;
            let { codecs: audioCodecs } = rStream.capabilities.audio
            let p = Promise.resolve();
            for (let i = 0; i < resolutions.length; i++) {
                let options = {
                    audio: {
                        codecs: [audioCodecs[i]]
                    },
                    video: {
                        codecs: [videocodecs[i]],
                        resolution: resolutions[i],
                        bitrateMultiplier: bitrateMultipliers[i],
                        frameRate: frameRates[i],
                        keyFrameInterval: keyFrameIntervals[i]
                    }

                }
                p = p.then(() => {
                    return new Promise(function (resolve, reject) {
                        sub.applyOptions(options).then(() => {
                            console.log('apply subscription success: ', options);
                            setTimeout(resolve, 5000);
                        }, err => {
                            console.log('apply subscription failed: ', options, err);
                            resolve();
                        })
                    })
                })
            ...
            p.then(() => {
                _stopLocal(lStream, lStream.source.video || lStream.source.audio);
                pub.stop();
                sub.stop();
            })
                .catch(e => {
...
function startIntegrateTest() {
    let createParams = ['camera', 'mic', resolutionName2Value['vga']];
    let pubParams = [getAudioAndVideoCodec('opus').audioCodec, getAudioAndVideoCodec(undefined, 'h264').videoCodec];
    let subParams = ['av', getAudioAndVideoCodec(undefined, 'h264').videoCodec, getAudioAndVideoCodec('opus').audioCodec, resolutionName2Value['vga']];

    let resolutions = ['sif', 'vga', 'hd720p', 'hd1080p'], audioCodec = ['opus', 'pcma', 'pcmu', 'g722-1', 'g722-2', 'isac16', 'isac32'],
        videoCodec = ['vp8', 'vp9', 'h264'];

    integrate(createParams, pubParams, subParams)
}

//setInterval(startIntegrateTest, 10*1000);
