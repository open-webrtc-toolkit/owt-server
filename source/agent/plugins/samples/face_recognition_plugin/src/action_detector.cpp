...
void ActionDetectionClass::submitRequest() {
...
void ActionDetectionClass::enqueue(const cv::Mat &frame) {
    if (!enabled()) return;
...
ActionDetectionClass::ActionDetectionClass(): BaseCnnDetection(false) {
    topoName = "action detector";
}

void ActionDetectionClass::initialize(const ActionDetectorConfig& config) {
    config_ = config;
    ...
    net_ = plugin.LoadNetwork(net_reader.getNetwork(), {});
...
void ActionDetectionClass::fetchResults() {
    if (!enabled()) return;
    results.clear();
...
inline ActionDetectionClass::NormalizedBBox
ActionDetectionClass::ParseBBoxRecord(
...
cv::Rect ActionDetectionClass::ConvertToRect(
...
void ActionDetectionClass::NonMaxSuppression(
