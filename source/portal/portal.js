...
const log = require('./logger').logger.getLogger('Portal');
const dataAccess = require('./data_access');
const {v4: uuid} = require('uuid');
const vsprintf = require('sprintf-js').vsprintf;

const Portal = function(spec, rpcReq) {
  const that = {};
  let token_key = spec.tokenKey;
  const cluster_name = spec.clusterName;
  const self_rpc_id = spec.selfRpcId;
  ...
  const calculateSignatureForWebTransportToken = (token) => {
    ...
    const signed =
        crypto.createHmac('sha256', token_key).update(toSign).digest('hex');
  ...
  that.validateAndDeleteWebTransportToken = (token) => {
    ...
    if (
      !Object.prototype.hasOwnProperty.call(participants, token.participantId)
    ) {
      return false;
    }
    if (
  ...
  that.join = function(participantId, token) {
    log.debug('participant[', participantId,
        '] join with token:', JSON.stringify(token));
    ...
    const validateToken = function(token) {
      const signature = calculateSignature(token);
        ...
        .then(function(joinResult) {
          ...
          if (token.webTransportUrl) {
            webTransportToken =
                (Buffer.from(
                    JSON.stringify(
                        generateWebTransportToken(participantId, room_id))))
                    .toString('base64');
          }
  ...
  that.publish = function(participantId, streamId, pubInfo) {
    log.debug('publish, participantId:', participantId,
        'streamId:', streamId,
        'pubInfo:', pubInfo);
  ...
  that.unpublish = function(participantId, streamId) {
    log.debug('unpublish, participantId:', participantId,
        'streamId:', streamId);
    if (participants[participantId] === undefined) {
      return Promise.reject('Participant has NOT joined');
    }
  ...
  that.streamControl = function(participantId, streamId, commandInfo) {
    log.debug('streamControl, participantId:', participantId,
        'streamId:', streamId,
        'command:', commandInfo);
    if (participants[participantId] === undefined) {
      return Promise.reject('Participant has NOT joined');
  ...
  that.subscribe = function(participantId, subscriptionId, subDesc) {
    log.debug('subscribe, participantId:', participantId,
  ...
  that.unsubscribe = function(participantId, subscriptionId) {
    log.debug('unsubscribe, participantId:', participantId,
        'subscriptionId:', subscriptionId);
    if (participants[participantId] === undefined) {
      return Promise.reject('Participant has NOT joined');
    }

    return rpcReq.unsubscribe(participants[participantId].controller,
        participantId,
        subscriptionId);
  };

  that.subscriptionControl =
      function(participantId, subscriptionId, commandInfo) {
        log.debug('subscriptionControl, participantId:', participantId,
            'subscriptionId:', subscriptionId,
            'command:', commandInfo);
        if (participants[participantId] === undefined) {
          return Promise.reject('Participant has NOT joined');
  ...
  that.onSessionSignaling = function(participantId, sessionId, signaling) {
    log.debug('onSessionSignaling, participantId:', participantId,
        'sessionId:', sessionId,
        'signaling:', signaling);
