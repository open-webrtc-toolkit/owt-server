...
const log = require('./logger').logger.getLogger('SocketIOServer');
...
const Connection = function(spec, socket, reconnectionKey, portal, dock) {
  const that = {
    socket: socket,
  };
  ...
  let waiting_for_reconnecting_timer = null;
  ...
  const run = () => {
      ...
      return validateUserAgent(login_info.userAgent)
          .then((reconnEnabled) => {
            ...
            return new Promise(function(resolve) {
              resolve(JSON.parse(
                  (new Buffer(login_info.token, 'base64')).toString()),
              );
            });
          }).then((token) => {
    ...
    socket.on('relogin', function(reconnectionTicket, callback) {
      ...
      let client;
      let reconnection_ticket;
      new Promise((resolve) => {
        resolve(
            JSON.parse((new Buffer(reconnectionTicket, 'base64')).toString()),
        );
      }).then((ticket) => {
        const now = Date.now();
...
const SocketIOServer = function(spec, portal, observer) {
  const that = {};
  let io;
  let clients = {};
  // A Socket.IO server has a unique reconnection key. Client cannot reconnect to another Socket.IO server in the cluster.
  const reconnection_key = require('crypto').randomBytes(64).toString('hex');
  const sioOptions = {};
  ...
  const startInsecure = function(port) {
  ...
  const startSecured = function(port, keystorePath, forceTlsv12) {
    return new Promise(function(resolve, reject) {
      const cipher = require('./cipher');
      const keystore = path.resolve(path.dirname(keystorePath), cipher.kstore);
      cipher.unlock(cipher.k, keystore, function(err, passphrase) {
        if (!err) {
          ...
          const server = require('https').createServer(option).listen(port);
          io = require('socket.io')(server, sioOptions);
          run();
          resolve('ok');
        } else {
  ...
  const run = function() {
    io.sockets.on('connection', function(socket) {
      const conn = Connection(spec, socket, reconnection_key, portal, that);
  ...
  that.notify = function(participantId, event, data) {
    log.debug('notify participant:', participantId,
        'event:', event,
        'data:', data);
  ...
  that.broadcast = function(controller, excludeList, event, data) {
    log.debug('broadcast controller:', controller,
        'exclude:', excludeList,
        'event:', event,
        'data:', data);
    portal.getParticipantsByController('node', controller)
        .then(function(receivers) {
          for (const clientId of receivers) {
