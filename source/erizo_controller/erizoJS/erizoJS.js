...
rpc.connect(function () {
    ...
    try {
        ...
        if (purpose === 'audio') {
            ...
            Controller = require('./videoNode').VideoNode;
        } else if (purpose === 'webrtc' || purpose === 'rtsp' || purpose === 'file') {
            Controller = require('./accessNode').AccessNode;
        } else  {
            log.error("Error", "Ambiguous purpose to start ErizoJS");
            return;
        }
        ...
        ejsController.keepAlive = function(callback) {
            //log.debug("KeepAlive from roomController");
