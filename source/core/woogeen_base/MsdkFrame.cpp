...
bool MsdkFrame::convertFrom(webrtc::I420VideoFrame& frame)
{
    ...
    // set luminance plane
    for(i = 0; i < h; i++) {
        memcpy_f(ptr + i * pitch, pSrcY + i * srcStrideY, w);
    }
...
bool MsdkFrame::nv12ConvertTo(mfxFrameInfo& pInfo, mfxFrameData& pData, webrtc::I420VideoFrame& frame)
{
    ...
    memcpy_f(m_nv12TBuffer, pData.Y, h * pData.Pitch);
    memcpy_f(m_nv12TBuffer + h * pData.Pitch, pData.UV, h * pData.Pitch / 2);
    ...
    // set luminance plane
    for(i = 0; i < h; i++) {
        memcpy_f(pDstY + i * dstStrideY, ptrY + i * pData.Pitch, w);
    }
...
void *MsdkFrame::memcpy_f(void *dst, void *src, size_t size)
{
    bool aligned;
    size_t remain;
    size_t i, round;
    ...
    if ( aligned != 0 ) {
        ELOG_WARN( "Addr is not 16 aligned, do normal copy instead: %p -> %p", src, dst );
