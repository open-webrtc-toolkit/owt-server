...
DEFINE_LOGGER(RawTransport, "ooVoo.RawTransport");
...
RawTransport::~RawTransport()
{
    ...
    // One assumption is that the AVS should send a message which is
    // interpreted by the ooVoo lib that we need to close the connection
    // (e.g., onClientLeave).
    // Update from ooVoo as of 20140211: the AVS won't send any message
    // back when the webrtc client leaves, so we shouldn't wait for the
    // network thread here because it's waiting for the AVS response.
    // We need to change it back if the AVS changes its behavior to send
    // a response on client leave, which we think might be more reasonable.
    if (m_tcpSocket)
        m_tcpSocket->close();
    if (m_udpSocket)
        m_udpSocket->close();
...
void RawTransport::createConnection(const std::string& ip, uint32_t port, Protocol prot)
{
    ...
    case TCP: {
        if (m_tcpSocket) {
            ELOG_WARN("TCP transport existed for this OoVooConnection, ignoring the connection request for ip %s port %d\n", ip.c_str(), port);
        } else {
    ...
    case UDP: {
        if (m_udpSocket) {
            ELOG_WARN("UDP transport existed for this OoVooConnection, ignoring the connection request for ip %s port %d\n", ip.c_str(), port);
        } else {
...
void RawTransport::connectHandler(Protocol prot, const boost::system::error_code& ec)
{
    if (!ec) {
        switch (prot) {
        case TCP:
            ELOG_DEBUG("Local TCP port for AVS connection: %d", m_tcpSocket->local_endpoint().port());
            // Disable Nagle's algorithm in the underlying TCP stack.
            // FIXME: Re-enable it later if we prove that AVS can correctly handle TCP reassembly
            // because that should improve the transmission efficiency.
            m_tcpSocket->set_option(tcp::no_delay(true));
            break;
        case UDP:
            ELOG_DEBUG("Local UDP port for AVS connection: %d", m_udpSocket->local_endpoint().port());
            break;
        default:
            break;
        }
