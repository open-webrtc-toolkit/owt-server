/*global window, console, clearInterval, setInterval, document, unescape, Woogeen, webkitURL*/
/*
 * VideoPlayer represents a Woogeen video component that shows either a local or a remote video.
...
Erizo.VideoPlayer = function (spec) {
  ...
  if (that.options !== undefined && that.options.muted !== undefined) {
  ...
  // Bottom Bar
  that.bar = new Erizo.Bar({elementID: 'player_' + that.id, id: that.id, stream: spec.stream, video: that.video, options: that.options});
