...
exports.RoomController = function (spec, on_init_ok, on_init_failed) {
    ...
    var shrinkStream = function (stream_id, target_erizo_id, subscriptions_reserved) {
        if (subscriptions_reserved === null || subscriptions_reserved === undefined) {
            subscriptions_reserved = false;
        }
        ...
        if (i !== -1) {
            ...
            makeRPC(
                ...
                'ErizoJS_' + target_erizo_id,
                'unpublish',
                [stream_id, subscriptions_reserved]);
    ...
    var unsubscribeStream = function (subscriber, subscription_id, reserved) {
        ...
        if (audio_stream) {
            ...
            if (!isSpreadNeeded(audio_stream, erizo_id)) {
                shrinkStream(audio_stream, erizo_id, reserved);
            }
        }
        ...
        if (video_stream) {
            ...
            if ((!audio_stream || video_stream !== audio_stream)
                ...
                shrinkStream(video_stream, erizo_id, reserved);
            }
        }
        ...
        if (!reserved && isTerminalFree(subscriber)) {
    ...
    that.unsubscribe = function (terminal_id, subscription_id, reserved) {
        if (reserved === null || reserved === undefined) {
            reserved = false;
        }
        ...
        if (terminals[terminal_id]) {
            unsubscribeStream(terminal_id, subscription_id, reserved);
        }
    };
